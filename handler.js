...
module.exports.getResults = async (event, context, callback) => {  
  ...
  try {
    ...
    console.log(date)
    const sql = "select symbol, indicator_level, avg_return, win_rate, avg_return, avg_win, avg_loss, observation_period from backtest_results_stg where todays_date = $1 order by symbol, cast(left(observation_period, position(' ' IN observation_period)-1) as int)"
